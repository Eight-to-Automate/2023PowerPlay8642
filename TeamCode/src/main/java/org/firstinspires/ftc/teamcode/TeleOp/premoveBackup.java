...
//exponential drive variables
    ...
    @Override
    public void loop() {
        // If a nudge button is pressed (ABXY bumpers) and the robot has no more actions to execute then enable nudge functions
        if(gamepad1.right_bumper || gamepad1.left_bumper || gamepad1.b || gamepad1.x || gamepad1.a || gamepad1.y && order.isEmpty() && !gamepad1.start) {
            //currstateDrive = DriveMode.Nudge;
            Nudge = true;
        }
        ...
        //if the nudge functions are enabled then nudge the robot

        if (Nudge == true && !gamepad1.start) {
            ...
            if (gamepad1.left_bumper) { //rotateLeft
                robot.frontLeftMotor.setPower(0.4);
                robot.frontRightMotor.setPower(-0.4);
                robot.backLeftMotor.setPower(0.4);
                robot.backRightMotor.setPower(-0.4);
            ...
            if (gamepad1.right_bumper) { //rotateRight
                robot.frontLeftMotor.setPower(-0.4);
                robot.frontRightMotor.setPower(0.4);
                robot.backLeftMotor.setPower(-0.4);
                robot.backRightMotor.setPower(0.4);
        ...
        //if the nudge buttons are not pressed then stop all drive motors and disable nudge
        if(!gamepad1.right_bumper || !gamepad1.left_bumper || !gamepad1.b || !gamepad1.x || !gamepad1.a || !gamepad1.y) {
